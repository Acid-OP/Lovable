...
export function startCleanupWorker() {
  ...
  const gracefulShutdown = async () => {
    logger.info('cleanup.worker.shutdown', { message: 'Graceful shutdown initiated' });
    clearInterval(intervalId);
    
    try {
      // Kill all running containers
      await cleanupAllContainers();
      logger.info('cleanup.worker.shutdown.complete', { message: 'All containers cleaned up' });
    } catch (error) {
  ...
  logger.info('cleanup.worker.started', { message: 'Cleanup worker is running' });
}
...
async function runCleanup() {
  ...
  for (const key of keys) {
    try {
      ...
      const lastActivity = parseInt(session.lastActivity || session.createdAt || '0');
      ...
      if (age > MAX_CONTAINER_AGE) {
        await killContainer(jobId, session.containerId, 'max_age_exceeded', age);
        killedCount++;
        continue;
      }
...
async function killContainer(jobId: string, containerId: string, reason: string, timeMs: number) {
  try {
