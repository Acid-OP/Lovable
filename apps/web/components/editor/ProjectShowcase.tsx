...
export function ProjectShowcase({ isDark }: ProjectShowcaseProps) {
  ...
  return (
    <div
      ...
      style={{ maxWidth: 600 }}
    >
      {/* Card + vertical loader side by side */}
      <div className="flex items-stretch gap-3 w-full">
        {/* Vertical progress loader — tight beside card */}
        <div
          className="relative flex flex-col items-center py-2"
          style={{ width: 6 }}
        >
          <div
            className={`w-[1.5px] h-full ${isDark ? "bg-[#2A2A2A]" : "bg-[#e0e0dc]"} rounded-full relative overflow-hidden`}
          >
            <div
              className={`absolute top-0 left-0 w-full rounded-full ${isDark ? "bg-[#555]" : "bg-[#bbb]"}`}
              style={{ animation: "progressFill 50s linear forwards" }}
            />
          </div>
          {[12, 37, 62, 87].map((pos) => (
            <div
              key={pos}
              className={`absolute w-[5px] h-[5px] rounded-full ${isDark ? "bg-[#444]" : "bg-[#ccc]"}`}
              style={{
                top: `${pos}%`,
                left: "50%",
