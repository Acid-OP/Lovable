      ...
      >
        ...
        <div className="relative w-full overflow-hidden rounded-t-xl">
          {project.image ? (
            ...
            >
              <div
                className={`flex items-center gap-1.5 px-4 py-2.5 border-b ${isDark ? "border-gray-200/10" : "border-gray-200"}`}
              >
                <div className="w-2.5 h-2.5 rounded-full bg-red-400/70" />
                <div className="w-2.5 h-2.5 rounded-full bg-yellow-400/70" />
                <div className="w-2.5 h-2.5 rounded-full bg-green-400/70" />
                <div
                  className={`ml-3 flex-1 h-4 rounded ${isDark ? "bg-white/10" : "bg-black/5"}`}
                />
              </div>
              <div className="flex-1 p-4 space-y-3">
                <div
                  className={`h-3.5 rounded w-1/2 ${isDark ? "bg-white/10" : "bg-black/8"}`}
                />
                <div
                  className={`h-2.5 rounded w-full ${isDark ? "bg-white/5" : "bg-black/5"}`}
                />
                <div
                  className={`h-2.5 rounded w-4/5 ${isDark ? "bg-white/5" : "bg-black/5"}`}
                />
                <div className="flex gap-2 mt-4">
                  <div
                    className={`h-16 flex-1 rounded-lg ${isDark ? "bg-white/8" : "bg-black/5"}`}
                  />
                  <div
                    className={`h-16 flex-1 rounded-lg ${isDark ? "bg-white/5" : "bg-black/[0.03]"}`}
                  />
                </div>
              </div>
            </div>
